...
export default function StudentDashboard() {
  ...
  const subjectEnglishRemaining = Math.max(0, subjectClassLimit - englishCompleted);
  const subjectMathematicsRemaining = Math.max(0, subjectClassLimit - mathematicsCompleted);
  const subjectPackageRemaining = subjectEnglishRemaining + subjectMathematicsRemaining;
  const subjectPackageTotal = subjectClassLimit * 2;
  ...
  return <DashboardLayout navItems={navItems}>
    ...
    <div className="relative flex-1 overflow-auto px-8 py-7">
      ...
      {isSubjectBalanceStudent && <section className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-2">
        <SubjectClassBox subject="English Classes" completed={englishCompleted} total={subjectClassLimit} />
        <SubjectClassBox subject="Mathematics Classes" completed={mathematicsCompleted} total={subjectClassLimit} />
      </section>}
      ...
      <section className="mb-8 rounded-2xl border border-chalk-faint bg-panel p-5 md:p-6">
        ...
        {pkg && <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3"><PackageStat label="Total classes" value={isSubjectBalanceStudent ? subjectPackageTotal : pkg.total_classes}/><PackageStat label="Completed classes" value={isSubjectBalanceStudent ? Math.min(subjectPackageTotal, (englishCompleted + mathematicsCompleted)) : pkg.completed_classes}/><PackageStat label="Remaining classes" value={isSubjectBalanceStudent ? subjectPackageRemaining : pkg.remaining_classes}/></div>}
      </section>
