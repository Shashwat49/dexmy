...
export default function Signup() {
  ...
  return <AuthLayout title={role === "teacher" ? "Create your teacher account" : "Create your account"}>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div><span className="block text-sm font-medium mb-1.5">I am a...</span><div className="flex gap-2 bg-panel-2 p-1 rounded-lg" role="group" aria-label="Account type">{ROLES.map((r)=><button type="button" key={r.value} onClick={()=>setRole(r.value)} aria-pressed={role===r.value} className={`flex-1 py-2 rounded-md text-sm font-semibold transition-colors ${role===r.value?"bg-brand-red text-chalk":"text-chalk-muted hover:text-chalk"}`}>{r.label}</button>)}</div></div>
      <div><label htmlFor="signup-name" className="block text-sm font-medium mb-1.5">Your name</label><input id="signup-name" name="full_name" type="text" required autoComplete="name" value={fullName} onChange={e=>setFullName(e.target.value)} placeholder={role==="parent"?"e.g. Sunita Mehta":"e.g. Aarav Mehta"} className="w-full bg-panel-3 border border-chalk-faint rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:border-brand-gold"/></div>
      <div><label htmlFor="signup-email" className="block text-sm font-medium mb-1.5">Email</label><input id="signup-email" name="email" type="email" required autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full bg-panel-3 border border-chalk-faint rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:border-brand-gold"/></div>
      <div><label htmlFor="signup-phone" className="block text-sm font-medium mb-1.5">Phone <span className="text-brand-red">*</span></label><div className="flex gap-2"><select id="signup-country-code" name="country_code" aria-label="Country calling code" value={countryCode} onChange={e=>setCountryCode(e.target.value)} className="w-[125px] shrink-0 bg-panel-3 border border-chalk-faint rounded-lg px-3 py-2.5 text-sm">{COUNTRY_CODES.map(c=><option key={`${c.country}-${c.code}`} value={c.code}>{c.flag} {c.code}</option>)}</select><input id="signup-phone" name="phone" type="tel" required autoComplete="tel" value={phone} onChange={e=>setPhone(e.target.value.replace(/\D/g,""))} placeholder="9876543210" className="flex-1 min-w-0 bg-panel-3 border border-chalk-faint rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:border-brand-gold"/></div></div>
      <div><label htmlFor="signup-password" className="block text-sm font-medium mb-1.5">Password</label><div className="relative"><input id="signup-password" name="password" type={showPassword?"text":"password"} required minLength={8} autoComplete="new-password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full bg-panel-3 border border-chalk-faint rounded-lg px-3.5 py-2.5 pr-11 text-sm focus:outline-none focus:border-brand-gold"/><button type="button" onClick={()=>setShowPassword(v=>!v)} aria-label={showPassword?"Hide password":"Show password"} className="absolute right-3 top-1/2 -translate-y-1/2 text-chalk-muted">{showPassword?"◉":"◌"}</button></div><p className="text-xs text-chalk-muted mt-1.5">Use at least 8 characters with uppercase, lowercase, and a number.</p></div>
