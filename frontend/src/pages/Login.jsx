      ...
      if (STAFF_ROLES.includes(user.role)) {
        ...
        setError(
          user.role === "test_creator"
            ? "Test Creator accounts must use the Test Creator sign-in portal."
            : "Admin and staff accounts must use the Admin sign-in portal."
        );
        return;
      }
    ...
    <AuthLayout title="Welcome back" subtitle="Log in to your Dexmy account">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input id="login-email" name="email" type="email" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full bg-panel-3 border border-chalk-faint rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:border-brand-gold" />
        </div>
        <div>
          <label htmlFor="login-password" className="block text-sm font-medium mb-1.5">Password</label>
          <div className="relative">
            <input id="login-password" name="password" type={showPassword ? "text" : "password"} required autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-panel-3 border border-chalk-faint rounded-lg px-3.5 py-2.5 pr-11 text-sm focus:outline-none focus:border-brand-gold" />
