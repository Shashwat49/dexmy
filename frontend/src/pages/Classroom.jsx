...
export default function Classroom() {
  ...
  const addSlide = () => { if (!isTeacher) return; const current = slideRef.current; saveSnapshotNow(current); const next = slidesRef.current.length + 1; const newPage = makeWhiteboardPage(next); strokesByPageRef.current.set(newPage.page_id, []); const updated = [...slidesRef.current, newPage]; slidesRef.current = updated; setSlides(updated); slideRef.current = next; setSlide(next); slideControlActiveRef.current = true; publishControl({ kind: "slides", pages: updated, page_number: next }); };
  ...
  const clearBoard = () => { if (!isTeacher) return; strokesByPageRef.current.set(currentPageId(), []); redraw(); send({ type: "whiteboard_event", payload: { kind: "clear", page_number: slideRef.current } }); saveSnapshot(); };
