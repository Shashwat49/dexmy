...
function readCachedState() {
  try {
    return sessionStorage.getItem(PROFILE_CHECK_KEY) === "true" ? "complete" : null;
  } catch { return null; }
}

export default function TeacherProfileGuard({ children }) {
  const [state, setState] = useState(() => readCachedState() || "checking");

  useEffect(() => {
    if (state === "complete") return;
    let cancelled = false;
    getMyTeacherProfile().then((profile) => {
      if (cancelled) return;
      if (isComplete(profile)) {
        try { sessionStorage.setItem(PROFILE_CHECK_KEY, "true"); } catch {}
        setState("complete");
