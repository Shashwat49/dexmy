...
export default function DashboardLayout({ navItems = [], children }) {
  ...
  const nav = () => <>
    <div className="flex items-center justify-between mb-8"><Link to="/" onClick={() => setMobileOpen(false)} className="font-display text-2xl text-brand-red -skew-x-6 inline-block">Dexmy</Link><button onClick={() => setMobileOpen(false)} className="md:hidden text-chalk-muted p-1 rounded-lg hover:bg-panel-2" aria-label="Close menu">×</button></div>
