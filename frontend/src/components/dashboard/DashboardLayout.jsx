...
function DexmyLogo({ mobile = false }) {
  return <img src="/dexmy-logo-bg-removed.png" alt="Dexmy" className={mobile ? "h-8 w-auto max-w-[120px] object-contain" : "h-9 w-auto max-w-[145px] object-contain"} />;
}
...
export default function DashboardLayout({ navItems = [], children }) {
  ...
  const nav = () => <>
    <div className="flex items-center justify-between mb-5 shrink-0"><Link to="/" onClick={() => setMobileOpen(false)} className="inline-flex items-center shrink-0" aria-label="Dexmy home"><DexmyLogo /></Link><button onClick={() => setMobileOpen(false)} className="md:hidden text-chalk-muted p-1 rounded-lg hover:bg-panel-2">×</button></div>
    <nav className="flex-1 min-h-0 overflow-hidden pr-1">{effectiveNav.map((section) => <div key={section.label} className="mb-3 last:mb-0"><div className="text-[10px] font-bold tracking-[0.16em] uppercase text-chalk-muted/60 mb-1 ml-2">{section.label}</div><div className="space-y-0.5">{section.items.map((item) => { const active = location.pathname === item.path || (item.path !== "/dashboard/admin" && item.path !== "/dashboard/teacher" && location.pathname.startsWith(`${item.path}/`)); return <Link key={item.path} to={item.path} onClick={() => setMobileOpen(false)} className={`group flex items-center gap-3 px-3 py-2 rounded-xl text-[14px] font-medium transition-all ${active ? "bg-brand-red-soft text-chalk shadow-[inset_3px_0_0_#E4271C]" : "text-chalk-muted hover:bg-panel-2 hover:text-chalk"}`}><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg transition-colors ${active ? "bg-brand-red/15 text-brand-red" : "bg-panel-2 text-chalk-muted group-hover:text-chalk"}`}><NavIcon type={item.icon} /></span><span className="truncate">{item.label}</span></Link>; })}</div></div>)}</nav>
    <div className="shrink-0 pt-3">
      <button onClick={() => { setMobileOpen(false); logout(); }} className="flex items-center gap-3 px-3 py-2 rounded-xl text-[14px] font-medium text-chalk-muted hover:bg-panel-2 hover:text-chalk transition-colors text-left w-full"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-panel-2">↪</span><span>Log out</span></button>
      <div className="mt-3 pt-3 border-t border-chalk-faint flex items-center gap-2.5"><div className="w-9 h-9 rounded-full bg-brand-gold text-[#2C1E04] flex items-center justify-center font-bold text-sm shrink-0">{initials}</div><div className="min-w-0 flex-1"><div className="text-[13.5px] font-semibold truncate">{user?.full_name}</div><div className="text-xs text-chalk-muted capitalize truncate">{user?.role}</div></div></div>
    </div>
  </>;
  return <div className="min-h-screen flex flex-col md:flex-row bg-void text-chalk font-body"><header className="md:hidden flex items-center justify-between px-4 py-3 bg-panel/95 backdrop-blur border-b border-chalk-faint sticky top-0 z-30"><button onClick={() => setMobileOpen(true)} className="grid h-9 w-9 place-items-center rounded-lg border border-chalk-faint hover:bg-panel-2">☰</button><Link to="/" className="inline-flex items-center shrink-0" aria-label="Dexmy home"><DexmyLogo mobile /></Link><div className="w-8 h-8 rounded-full bg-brand-gold text-[#2C1E04] flex items-center justify-center font-bold text-xs">{initials}</div></header>{mobileOpen && <div className="fixed inset-0 z-50 flex md:hidden"><div className="fixed inset-0 bg-black/70 backdrop-blur-sm" onClick={() => setMobileOpen(false)} /><aside className="relative z-50 w-72 max-w-[88vw] bg-panel h-dvh flex flex-col p-5 sm:p-6 shadow-2xl border-r border-chalk-faint overflow-hidden">{nav()}</aside></div>}<aside className="hidden md:flex w-64 shrink-0 bg-panel border-r border-chalk-faint flex-col p-5 lg:p-6 h-dvh max-h-dvh sticky top-0 overflow-hidden">{nav()}</aside><main className="flex-1 flex flex-col min-w-0 overflow-x-hidden">{children}</main></div>;
}
