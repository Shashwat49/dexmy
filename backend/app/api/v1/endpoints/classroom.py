...
def get_join_token(
    ...
    # Teachers can publish all standard media sources. Students start with
    # camera/microphone only; screen-share is granted later by the teacher via
    # the authenticated classroom WebSocket + LiveKit room permission update.
    sources = None if is_teacher else ["camera", "microphone"]
...
def get_notes(session_id: uuid.UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    class_session = db.get(ClassSession, session_id)
    if class_session is None: raise HTTPException(status_code=404, detail="Session not found")
    booking = db.get(Booking, class_session.booking_id)
...
async def upload_whiteboard_pdf(session_id: uuid.UUID, file: UploadFile = File(...), current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    try: page_images = render_pdf_to_images(contents)
    except Exception: raise HTTPException(status_code=400, detail="Could not read PDF")
    if len(page_images) > 50: raise HTTPException(status_code=400, detail="PDF too long (50 pages max)")
    pages=[]
    ...
    room=manager.get_room(session_id); peer=room.student_ws
    if peer:
        await peer.send_json({"type":"pdf_pages_ready","pages":[{"page_number":i,"image_url":p.file_url} for i,p in enumerate(pages,1)]})
    return pages
