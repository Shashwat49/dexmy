...
import uuid
from app.models.classroom_content import ClassNotes
...
def get_join_token(payload:JoinTokenRequest,current_user:User=Depends(get_current_user),db:Session=Depends(get_db)):
    ...
    # Students receive a subscribe-only media token until the authenticated
    # classroom websocket confirms teacher admission. The websocket then uses
    # LiveKit's server-side UpdateParticipant API to grant camera/mic and later
    # any teacher-approved sources.
...
async def end_session(session_id:uuid.UUID,current_user:User=Depends(get_current_user),db:Session=Depends(get_db)):
    ...
    notes=end_class_session(session_id,db);room=manager.rooms.pop(session_id,None)
    if room:
...
async def upload_whiteboard_pdf(session_id:uuid.UUID,file:UploadFile=File(...),current_user:User=Depends(get_current_user),db:Session=Depends(get_db)):
    ...
    for i,img in enumerate(pages_raw,1):
        key=save_bytes_file(img,f"annotate_{session_id}_p{i}","png");pages.append(FileUploadResponse(file_url=get_presigned_url(key,expires_in=86400),file_name=f"page_{i}.png"))
    room=manager.get_room(session_id)
    if room.student_ws:
        await room.student_ws.send_json({"type":"pdf_pages_ready","pages":[{"page_number":i,"image_url":p.file_url} for i,p in enumerate(pages,1)]})
    return pages
