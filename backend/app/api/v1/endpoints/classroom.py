...
async def upload_whiteboard_pdf(session_id: uuid.UUID, file: UploadFile = File(...), after_page_id: str | None = None, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if cs is None: raise HTTPException(status_code=404, detail="Session not found")
    booking = db.get(Booking, cs.booking_id)
    if current_user.id != booking.teacher_id: raise HTTPException(status_code=403, detail="Only the teacher can upload annotation PDFs")
    if not file.filename.lower().endswith(".pdf"): raise HTTPException(status_code=400, detail="File must be a PDF")
    contents = await file.read()
    if len(contents) > 30 * 1024 * 1024: raise HTTPException(status_code=413, detail="PDF too large (30MB max)")
    try: pages_raw = render_pdf_to_images(contents)
    except Exception: raise HTTPException(status_code=400, detail="Could not read PDF")
    if len(pages_raw) > 50: raise HTTPException(status_code=400, detail="PDF too long (50 pages max)")

    existing_pages = db.query(ClassroomPage).filter(ClassroomPage.session_id == session_id).order_by(ClassroomPage.position.asc()).all()
    after_position = len(existing_pages)
    if after_page_id:
        try:
            target = db.get(ClassroomPage, uuid.UUID(after_page_id))
            if target and target.session_id == session_id: after_position = target.position
    ...
    inserted = []
    for offset, (i, key) in enumerate(stored, 1):
        page = ClassroomPage(session_id=session_id, position=after_position + offset, page_type="pdf", image_url=key)
        db.add(page); db.flush(); inserted.append(page)
        db.add(WhiteboardSnapshot(session_id=session_id, snapshot_data={"strokes": []}, image_url=key, page_number=page.position, page_id=page.id))
    db.commit()

    ordered = db.query(ClassroomPage).filter(ClassroomPage.session_id == session_id).order_by(ClassroomPage.position.asc()).all()
    payload_pages = [{"page_id": str(page.id), "page_number": i, "page_type": page.page_type, "image_url": get_presigned_url(page.image_url, expires_in=86400) if page.image_url else None} for i, page in enumerate(ordered, 1)]
    payload = {"type": "pdf_pages_ready", "pages": payload_pages, "inserted_page_ids": [str(page.id) for page in inserted]}
    room = manager.get_room(session_id)
    for ws in (room.teacher_ws, room.student_ws):
        if ws:
            try: await ws.send_json(payload)
            except Exception: pass
    return WhiteboardPdfUploadResponse(pages=[WhiteboardPageResponse(**p) for p in payload_pages], inserted_count=len(inserted))
