...
def review_profile_change_request(request_id:uuid.UUID,payload:dict,request:Request,current_user:User=Depends(require_permission("teacher.verify")),db:Session=Depends(get_db)):
    ...
    if decision=="rejected" and not str(reason or "").strip(): raise HTTPException(status_code=422,detail="A rejection reason is required")
    if decision=="approved":
        ...
        if "subject_ids" in changes:
            ids=list(dict.fromkeys(changes["subject_ids"] or [])); existing={row[0] for row in db.execute(select(Subject.id).where(Subject.id.in_(ids))).all()}
