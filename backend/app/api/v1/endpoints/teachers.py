...
def _validate_application(profile: TeacherProfile, subject_ids: list[int], user: User):
    ...
    if missing: raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail={"message":"Complete every required teacher profile field before submitting your application.","missing_fields":missing})
...
def get_my_pending_teacher_profile_change(current_user: User = Depends(require_role(UserRole.teacher)), db: Session = Depends(get_db)):
...
def update_my_teacher_profile(payload: TeacherProfileUpdate,current_user: User = Depends(require_role(UserRole.teacher)),db: Session = Depends(get_db)):
    ...
    if "subject_ids" in incoming:
        subject_ids=list(dict.fromkeys(incoming.pop("subject_ids") or []))
    ...
    if pending: raise HTTPException(status_code=status.HTTP_409_CONFLICT,detail="You already have a profile change awaiting admin review.")
    requested={**incoming,"subject_ids":subject_ids}
    db.add(TeacherProfileChangeRequest(teacher_id=current_user.id,requested_changes=requested,created_at=datetime.now(timezone.utc)))
    db.commit()
