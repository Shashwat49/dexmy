...
def get_or_create_payment(
    ...
    *,
    ...
    if existing is not None:
        if existing.payer_id != payer_id or existing.package_plan_id != package_plan.id or existing.provider.value != provider:
...
def activate_package_from_payment(db: Session, *, payment_id: uuid.UUID, provider_payment_id: str | None = None) -> StudentPackage:
    ...
    if payment.package_plan_id is None:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="Payment is not linked to a package plan")
    ...
    existing_package = db.execute(
        select(StudentPackage).where(StudentPackage.payment_id == payment.id).with_for_update()
    ).scalar_one_or_none()
