...
def booking_to_scheduling_booking(booking: Booking) -> SchedulingBooking:
    return SchedulingBooking(
        ...
        end=get_booking_end(booking.scheduled_at, booking.duration_minutes),
...
def get_active_bookings_for_interval(
    ...
    *,
    ...
    # The lower bound is important. The old query fetched every historical
    # pending/confirmed booking and filtered overlaps in Python.
    ...
    return [
        item
        for item in (booking_to_scheduling_booking(row) for row in query.all())
...
def get_active_bookings_for_slot(db: Session, slot_start: datetime) -> list[SchedulingBooking]:
    return get_active_bookings_for_interval(
        db, slot_start, slot_start + timedelta(minutes=SLOT_DURATION_MINUTES)
    )


def get_active_bookings_for_day(
    db: Session,
    start: datetime,
    end: datetime,
) -> list[SchedulingBooking]:
    """Load the booking data needed for an entire availability request once."""
...
def get_all_eligible_teacher_subjects(db: Session) -> dict[object, frozenset[int]]:
    """Load all active + verified teacher subject mappings in one query."""
    rows = (
...
def teacher_can_teach_booking(
    ...
    teacher_subject_map: Mapping[object, frozenset[int]],
) -> bool:
...
def find_maximum_matching(graph: dict[object, list[object]]) -> dict[object, object]:
    """Hopcroft-Karp maximum matching: faster than repeated DFS augmentation."""
    if not graph:
        return {}
    ...
    def bfs() -> bool:
        ...
        while queue:
            ...
            for teacher in graph.get(node, ()):
                matched_left = pair_right.get(teacher)
                if matched_left is None:
                    found = True
                elif distance.get(matched_left, -1) == -1:
                    distance[matched_left] = distance[node] + 1
    ...
    def dfs(node: object) -> bool:
        for teacher in graph.get(node, ()):
            ...
            if matched_left is None or (
                distance.get(matched_left, -1) == distance[node] + 1 and dfs(matched_left)
            ):
...
def _matching_for_bookings(
    bookings: Sequence[SchedulingBooking],
    teacher_subject_map: Mapping[object, frozenset[int]],
    extra_occupied: Iterable[object] = (),
) -> MatchingResult:
...
def calculate_slot_matching(
    ...
    *,
    ...
    subject_ids = {booking.subject_id for booking in bookings}
    teacher_subject_map = get_all_eligible_teacher_subjects(db)
    teacher_subject_map = {
        teacher_id: subjects
        for teacher_id, subjects in teacher_subject_map.items()
        if subjects & subject_ids
    }
    return _matching_for_bookings(bookings, teacher_subject_map)


def _capacity_from_preloaded(
    subject_id: int,
    bookings: Sequence[SchedulingBooking],
    teacher_subject_map: Mapping[object, frozenset[int]],
    slot_start: datetime,
) -> int:
    """Compute capacity with zero DB calls; feasibility is monotonic."""
    relevant_subjects = {booking.subject_id for booking in bookings} | {subject_id}
    teachers = {
        teacher_id: subjects
        for teacher_id, subjects in teacher_subject_map.items()
        if subjects & relevant_subjects
    }
    ...
    eligible_available = [
        teacher_id
        for teacher_id, subjects in teachers.items()
        if teacher_id not in occupied and subject_id in subjects
    ]
    ...
    def feasible(count: int) -> bool:
        synthetic = [
            SchedulingBooking(
                ...
                student_id=f"__capacity_student__:{i}",
                subject_id=subject_id,
                start=slot_start,
                end=slot_start + timedelta(minutes=CLASS_DURATION_MINUTES),
                teacher_id=None,
            )
            for i in range(count)
        ]
        result = _matching_for_bookings(list(bookings) + synthetic, teachers)
        return result.is_fully_assignable

    # Feasibility is monotonic: if N bookings fit, every number < N fits.
    lo, hi = 0, max_extra
    while lo < hi:
        mid = (lo + hi + 1) // 2
        if feasible(mid):
...
def get_slot_capacity(db: Session, subject_id: int, slot_start: datetime) -> int:
    bookings = get_active_bookings_for_slot(db, slot_start)
    teacher_subject_map = get_all_eligible_teacher_subjects(db)
    return _capacity_from_preloaded(subject_id, bookings, teacher_subject_map, slot_start)


def get_available_slot_capacities(
    db: Session,
    subject_id: int,
    slots: Sequence[datetime],
) -> list[int]:
    """Batch availability calculation used by the student booking endpoint."""
    if not slots:
        return []
    ...
    for slot_start in slots:
        ...
        slot_bookings = [
            ...
            if intervals_overlap(booking.start, booking.end, slot_start, slot_end)
        ]
...
def can_accept_booking(
    ...
    *,
    ...
    synthetic = SchedulingBooking(
        booking_id=f"new-booking-{student_id}-{subject_id}-{slot_start.isoformat()}",
    ...
    result = calculate_slot_matching(db, slot_start, additional_booking=synthetic)
...
def can_assign_teacher(
    ...
    *,
    ...
    slot_end = booking_start + timedelta(minutes=SLOT_DURATION_MINUTES)
    other_bookings = get_active_bookings_for_interval(
        db, booking_start, slot_end, exclude_booking_id=booking.id
    )
    ...
    teacher_subject_map = {
        tid: subjects
        for tid, subjects in teacher_subject_map.items()
        if tid != teacher_id and subjects & subject_ids
    }
