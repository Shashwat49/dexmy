    ...
    try:
        ...
        if is_teacher:
            ...
            if room.pending_student:
                ...
                room.permissions.setdefault(str(student_id), {"mic", "camera"})
                await room.student_ws.send_json({"type":"admitted"})
                try: await _sync_livekit_permissions(class_session, student_id, False, room)
        ...
        try:
            while True:
                ...
                await _handle_message(data, user, is_teacher, session_id, room, db, websocket, class_session)
...
async def _handle_message(data: dict, user: User, is_teacher: bool, session_id: uuid.UUID, room, db: Session, websocket: WebSocket, class_session: ClassSession):
    ...
    if msg_type == "chat":
        ...
        if data.get("image_base64"):
            image_url=save_base64_file(data["image_base64"],f"wb_{session_id}_p{data.get('page_number',1)}","png")
        db.add(WhiteboardSnapshot(session_id=session_id,snapshot_data=data.get("canvas_json",{}),image_url=image_url,page_number=data.get("page_number",1)))
        db.commit()
...
async def session_timer(session_id: uuid.UUID):
    while True:
        ...
        try:
            class_session=db.get(ClassSession,session_id)
            if class_session is None or class_session.status==SessionStatus.ended: return
        finally: db.close()
...
async def _auto_end_session(session_id: uuid.UUID, room):
    ...
    for ws in (room.teacher_ws,room.student_ws):
        if ws:
            try: await ws.send_json(payload_out)
            except Exception: pass
