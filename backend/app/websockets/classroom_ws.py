    ...
    async with api.LiveKitAPI(settings.LIVEKIT_URL,settings.LIVEKIT_API_KEY,settings.LIVEKIT_API_SECRET) as lkapi:
        await lkapi.room.update_participant(api.UpdateParticipantRequest(
            ...
            permission=api.ParticipantPermission(
                ...
                can_publish_sources=_allowed_sources(room,user_id,False),
            ),
        ))
    ...
    try:
        ...
        if is_teacher:
            ...
            if room.pending_student:
                ...
                sid=booking.student_id; room.permissions.setdefault(str(sid),{"mic","camera"})
...
async def _handle_message(data,user,is_teacher,session_id,room,db,websocket,class_session):
    ...
    if msg_type=="chat":
        ...
        if target_user_id != class_session.booking_id:
            # A classroom teacher may only modify the student belonging to this booking.
            if target_user_id != db.get(Booking,class_session.booking_id).student_id:
        ...
        db.add(WhiteboardSnapshot(session_id=session_id,snapshot_data=data.get("canvas_json",{}),image_url=image_url,page_number=data.get("page_number",1))); db.commit()
