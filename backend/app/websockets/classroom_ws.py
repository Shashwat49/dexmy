...
def _authenticate(token: str, db) -> User | None:
    try:
        payload = decode_access_token(token); user_id = uuid.UUID(payload["sub"])
    except (JWTError, KeyError, ValueError, TypeError): return None
    return db.get(User, user_id)

def _default_student_permissions() -> set[str]: return {"mic", "camera"}

def _allowed_sources(room, user_id: uuid.UUID) -> list[str]:
    permissions = room.permissions.get(str(user_id), set()); sources = []
...
async def _sync_livekit_permissions(class_session: ClassSession, user_id: uuid.UUID, room) -> None:
    ...
    try:
        await lkapi.room.update_participant(api.UpdateParticipantRequest(
            ...
            permission=api.ParticipantPermission(can_subscribe=True, can_publish=True, can_publish_data=True, can_publish_sources=_allowed_sources(room, user_id)),
        ))
    finally: await lkapi.aclose()

def _restore_student_permissions(session_id: uuid.UUID, student_id: uuid.UUID, db) -> set[str]:
    permissions = _default_student_permissions(); events = db.query(PermissionEvent).filter(PermissionEvent.session_id == session_id, PermissionEvent.target_user_id == student_id).order_by(desc(PermissionEvent.created_at)).all(); latest = {}
    for event in events:
        if event.permission not in latest: latest[event.permission] = event.granted
...
async def _send_latest_whiteboard(session_id: uuid.UUID, websocket: WebSocket, db) -> None:
    snapshots = db.query(WhiteboardSnapshot).filter(WhiteboardSnapshot.session_id == session_id).order_by(WhiteboardSnapshot.page_number.asc(), WhiteboardSnapshot.created_at.desc()).all(); latest_by_page = {}
    for snapshot in snapshots: latest_by_page.setdefault(snapshot.page_number, snapshot)
    if not latest_by_page: return
    pages = [{"page_number": n, "image_url": get_presigned_url(s.image_url, expires_in=3600)} for n, s in sorted(latest_by_page.items()) if s.image_url]
    current_page = max(latest_by_page); snapshot = latest_by_page[current_page]
...
async def classroom_socket(websocket: WebSocket, session_id: uuid.UUID, token: str = Query(...)):
    ...
    try:
        ...
        if class_session.status in (SessionStatus.ended, SessionStatus.cancelled): await websocket.close(code=4409); return
        ...
        if is_teacher:
            room.teacher_ws = websocket; room.permissions[str(user.id)] = {"annotate","screen_share","mic","camera"}
            if class_session.status == SessionStatus.scheduled: class_session.status = SessionStatus.live; class_session.started_at = func.now(); db.commit()
            if room.deadline is None:
                ...
                else: room.deadline = datetime.now(timezone.utc) + timedelta(minutes=CLASS_DURATION_MINUTES)
            if room.timer_task is None or room.timer_task.done(): room.timer_task = asyncio.create_task(session_timer(session_id))
            await websocket.send_json({"type":"class_started","deadline":room.deadline.isoformat(),"student_present":room.student_ws is not None,"student_id":str(booking.student_id) if room.student_ws else None})
            if room.student_ws:
                student = db.get(User, booking.student_id)
                if student: await websocket.send_json({"type":"participant_info","role":"student","name":student.full_name})
            if room.pending_student:
                room.student_ws = room.pending_student; room.pending_student = None; student_id = booking.student_id; room.permissions[str(student_id)] = _restore_student_permissions(session_id, student_id, db)
                await room.student_ws.send_json({"type":"admitted","deadline":room.deadline.isoformat()}); await room.student_ws.send_json({"type":"participant_info","role":"teacher","name":user.full_name})
            ...
            if room.teacher_ws is None:
                room.pending_student = websocket; await websocket.send_json({"type":"waiting_for_teacher"})
            else:
                room.student_ws = websocket; await websocket.send_json({"type":"admitted","deadline":room.deadline.isoformat() if room.deadline else None})
                teacher = db.get(User, booking.teacher_id)
                if teacher: await websocket.send_json({"type":"participant_info","role":"teacher","name":teacher.full_name})
...
async def _handle_message(data, user, is_teacher, session_id, room, db, websocket, class_session, booking):
    ...
    if msg_type == "chat":
        ...
        except (KeyError, ValueError, TypeError): await websocket.send_json({"type":"permission_denied","reason":"Invalid permission request"}); return
        ...
        db.add(PermissionEvent(session_id=session_id,target_user_id=target_user_id,permission=permission,granted=granted,granted_by=user.id)); db.commit(); key=str(target_user_id); room.permissions.setdefault(key,_default_student_permissions())
        ...
        if peer: await peer.send_json(event)
        ...
        if not is_teacher and "annotate" not in room.permissions.get(str(user.id), set()): await websocket.send_json({"type":"permission_denied","permission":"annotate"}); return
        page_number=max(1,int(data.get("page_number",1))); existing=db.query(WhiteboardSnapshot).filter(WhiteboardSnapshot.session_id==session_id,WhiteboardSnapshot.page_number==page_number).order_by(desc(WhiteboardSnapshot.created_at)).first(); image_url=existing.image_url if existing else None
        if data.get("image_base64"): image_url=save_base64_file(data["image_base64"],f"wb_{session_id}_p{page_number}","png")
        db.add(WhiteboardSnapshot(session_id=session_id,snapshot_data=data.get("canvas_json") or {},image_url=image_url,page_number=page_number)); db.commit(); await websocket.send_json({"type":"snapshot_saved","page_number":page_number})
    elif msg_type == "extend_class" and is_teacher:
        if room.extended: await websocket.send_json({"type":"extend_denied","reason":"Already extended once"}); return
        room.extended=True; room.deadline=(room.deadline or datetime.now(timezone.utc))+timedelta(minutes=5); out={"type":"class_extended","new_deadline":room.deadline.isoformat()}
        for ws in (room.teacher_ws,room.student_ws):
            if ws: await ws.send_json(out)
    elif msg_type == "leave": await websocket.close(code=1000)
